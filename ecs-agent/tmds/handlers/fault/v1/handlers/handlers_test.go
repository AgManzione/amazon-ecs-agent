...
func setStartTCFaultExpectations(
	...
	for _, interfaceName := range interfaces {
		...
		// Add 3rd layer hash filter. parent, handle and baseclass change so accepting gomock.any()
...
func generateStartNetworkLatencyTestCases() []networkFaultInjectionTestCase {
	...
	tcs := []networkFaultInjectionTestCase{
		...
		},
		{
			...
			setExecExpectations: func(exec *mock_execwrapper.MockExec, ctrl *gomock.Controller) {
				latencyNetemArguments := fmt.Sprintf("delay %dms %dms", delayMilliseconds, jitterMilliseconds)
				setStartTCFaultExpectations(exec, ctrl, []string{"eth0", "eth1"}, latencyNetemArguments, ipSourcesToFilter, ipSources, flowsPercent, noPrefix)
			},
			expectedResponseJSON: happyFaultRunningResponse,
		},
		{
	...
	tcs := []networkFaultInjectionTestCase{
		{
			...
			setExecExpectations: func(exec *mock_execwrapper.MockExec, ctrl *gomock.Controller) {
				...
				setStartTCFaultExpectations(exec, ctrl, []string{"eth0", "eth1"}, packetLossNetemArguments, ipSourcesToFilter, ipSources, flowsPercent, noPrefix)
			},
			expectedResponseJSON: happyFaultRunningResponse,
		},
		{
